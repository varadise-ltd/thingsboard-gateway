...
from bluepy.btle import DefaultDelegate, Peripheral, Scanner, UUID, capitaliseName, BTLEInternalError, ScanEntry, Characteristic, ADDR_TYPE_PUBLIC
...
class DeviceProcessor(Thread):
    ...
    def __init__(self, device: ScanEntry, device_config, connector: BLEConnector):
        super().__init__()
    ...
    def __prepare_config(self):
        ...
        if self.__device_config.get('converter') is not None:
            converters[self.__device_config['converter']] = TBModuleLoader.import_module(self.__connector.connector_type, self.__device_config['converter'](self.__device_config))
            ...
            for key_config in self.__device_config[config_section]:
                if key_config.get('characteristicUUID') is not None:
                    if key_config.get('converter') is not None:
                        converters[key_config['converter']] = TBModuleLoader.import_module(self.__connector.connector_type, key_config['converter'])(self.__device_config)
    ...
    def __new_device_processing(self):
        default_services_on_device = [service for service in self.services.keys() if
                                      int(service.split('-')[0], 16) in BLEConnector.DEFAULT_SERVICES_LIST]
    ...
    def send_to_converter(self, config, data):
        self.__connector.add_data_task_to_queue(data, config, self.uplink_converters[config[self.SECTION_CONFIG_PARAMETER].get('converter', 'BytesBLEUplinkConverter')])
...
class ScanDelegate(DefaultDelegate):
    def __init__(self, ble_connector: BLEConnector):
        DefaultDelegate.__init__(self)
